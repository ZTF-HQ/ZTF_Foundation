...
// Routes protégées - Après authentification, vérifier le matricule
Route::middleware('auth')->group(function () {
    // Routes pour la vérification d'identite (matricule)
...
// Afficher le formulaire en 9 étapes (accessible à tous - pas d'authentification requise)
Route::get('/formulaire/create', [HqStaffFormController::class, 'showBigForm'])->name('BigForm');

// Modifier le formulaire (pour les utilisateurs authentifiés)
Route::get('/formulaire/{formId}/edit', [HqStaffFormController::class, 'editForm'])->middleware('auth')->name('staff.form.edit');

// Enregistrer et traiter la soumission du formulaire (POST protégée)
Route::post('/formulaire/store', [HqStaffFormController::class, 'store'])->name('staff.form.store');

// Télécharger le formulaire depuis le dashboard
Route::get('/staff/form/{formId}/download', [StaffDashboardController::class, 'downloadForm'])->middleware('auth')->name('staff.form.download');

// Télécharger le PDF (appelé après soumission du formulaire - PUBLIC)
Route::post('/download-pdf', [HqStaffFormController::class, 'telechargerPDF'])->name('download.pdf');

// Afficher le PDF généré publiquement (pas d'authentification requise)
Route::get('/public-pdf/{filename}', function ($filename) {
    $path = storage_path('app/public/pdfs/staff_forms/' . $filename);
...
// Routes pour les PDFs (protégées par authentification)
Route::middleware('auth')->group(function () {
...
// NOUVELLE ROUTE IMPORTANTE - Pour enregistrer les données dans la base de données
Route::post('/formulaire/store', [HqStaffFormController::class, 'store'])->name('staff.form.store');
