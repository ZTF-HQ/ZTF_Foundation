<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cr̩er un Service - ZTF Foundation</title>
    
    <link rel="stylesheet" href="{{ asset('css/create.css') }}">
</head>
<body>
    <div class="container">
        @if (!Auth::user()->isAdmin2() && !Auth::user()->isSuperAdmin() && !Auth::user()->isAdmin1() && !(str_starts_with(Auth::user()->matricule, 'CM-HQ-') && str_ends_with(Auth::user()->matricule, '-CD')))
            <div class="alert">
                <strong>Acc̬s non autoris̩!</strong>
                <span>Seuls les chefs de d̩partement peuvent cr̩er des services.</span>
            </div>
        @else
            <div class="header">
                <h1>Cr̩er un nouveau service</h1>
                <p>Remplissez les informations ci-dessous pour cr̩er un service</p>
            </div>

            @if ($errors->any())
                <div class="error-list">
                    <strong>Erreurs!</strong>
                    <ul>
                        @foreach ($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif

            <form action="{{ route('committee.services.store') }}" method="POST">
                @csrf

                <div class="form-group">
                    <label for="name">Nom du service*</label>
                    <input 
                        type="text" 
                        id="name" 
                        name="name" 
                        value="{{ old('name') }}" 
                        required 
                        placeholder="Entrez le nom du service"
                    >
                </div>

                <div class="form-group">
                    <label for="description">Description*</label>
                    <textarea 
                        id="description" 
                        name="description" 
                        rows="4" 
                        required 
                        placeholder="D̩crivez le r̫le et les responsabilit̩s du service"
                    >{{ old('description') }}</textarea>
                </div>

                <div class="form-group">
                    <label for="department_id">D̩partement*</label>
                    <select 
                        id="department_id" 
                        name="department_id" 
                        required
                    >
                        <option value="">S̩lectionnez un d̩partement</option>
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ old('department_id') == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
                    <div class="help-text">S̩lectionnez le d̩partement auquel ce service sera rattach̩</div>
                </div>

                <div class="buttons">
                    <a href="{{ route('committee.services.index') }}" class="btn btn-cancel">
                        Annuler
                    </a>
                    <button type="submit" class="btn btn-submit">
                        Cr̩er le service
                    </button>
                </div>
            </form>
        @endif
    </div>
</body>
</html>
